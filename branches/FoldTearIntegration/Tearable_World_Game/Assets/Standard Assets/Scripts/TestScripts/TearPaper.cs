...
public class TearPaper : MonoBehaviour {
	...
	private void CheckForBadTear()
	{
		if(!PlatformPaper)
		{
			for(int itor = 0; itor < CurrentCuttPiece.GetComponent<MeshFilter>().mesh.triangles.Count(); itor++)
			{
				...
				if(GameObject.FindGameObjectWithTag("Player").GetComponent<SphereCollider>().bounds.Contains(worldVertPos))
				{
					PlayerTornUntearableObjects();
					return;
				}
				
				worldVertPos.z = GameObject.FindGameObjectWithTag("Collectable").transform.position.z;
				if(GameObject.FindGameObjectWithTag("Collectable").GetComponent<SphereCollider>().bounds.Contains(worldVertPos))
				{
					PlayerTornUntearableObjects();
					return;
				}
	...
	/// </summary>
	public void ForceStopTear()
	{
		if(!PlatformPaper) Debug.LogError("bad tear being reset - ForceStopTear()");
		...
		*/
		...
		if(island2.Count > island1.Count)
		{
			...
			if(!PlatformPaper)
			{
				...
				//GetComponent<ClosestVertice>().CutPiece = Clone2;
			}
		...
		else
		{
			...
			if(!PlatformPaper)
			{
				...
				//GetComponent<ClosestVertice>().CutPiece = Clone;
			}

...
public class SoundManager : MonoBehaviour {
	...
	#region AudioClip's
	private AudioClip[] sfxClips;// = new AudioClip[20];
	private AudioClip[] musicClips;// = new AudioClip[10];
	#endregion
	private AudioSource[] sfxSounds;// = new AudioSource[20];
	private AudioSource[] musicSounds;// = new AudioSource[10];
	/// <summary>
	...
	private void SetVolForSpecificSnd(float vol, string sndName, AudioSource[] s, int len)
	{
		...
		for(int i = 0; i < len; i++)
		{
			...
			{
				s[i].volume = vol;
				sndFound = true; //make it known the sound was located
	...
	private void PlaySFXorMusic(AudioSource[] s, int len, string sndName)
	{
		...
		if(!sndFound) Debug.Log (sndName + " not found for PlayAudio");
	}
	/// <summary>

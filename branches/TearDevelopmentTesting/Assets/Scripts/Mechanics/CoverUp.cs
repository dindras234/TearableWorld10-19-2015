...
public class CoverUp : MonoBehaviour {
	...
	void Update () {
		...
		if(touchType == TouchType.SWIPE || touchType == TouchType.DRAG){
			fingerPositions = touchController.GetFingerPositions();
			fingerPosition = Camera.main.ScreenToWorldPoint(fingerPositions[0]);
			
			//float tempfpx = fingerPosition.x - 1;
			//float tempfpy = fingerPosition.y;
			//float foldX = (fingerPosition.y*fingerPosition.y/(2*fingerPosition.x)) + (fingerPosition.x/2);
			//float foldY = (fingerPosition.x*fingerPosition.x/(2*fingerPosition.y)) + (fingerPosition.y/2);
			//float foldX = (tempfpy*tempfpy/(2*tempfpx)) + (tempfpx/2);
			//float foldY = (tempfpx*tempfpx/(2*tempfpy)) + (tempfpy/2);
			//this.transform.position = origin;
			this.transform.rotation = originalRotation;
		
	    	this.transform.position = new Vector3(fingerPosition.x/2,fingerPosition.y/2, 0);
		
	    	this.transform.Rotate(Vector3.forward, Mathf.Rad2Deg * Mathf.Atan2(fingerPosition.y, fingerPosition.x));

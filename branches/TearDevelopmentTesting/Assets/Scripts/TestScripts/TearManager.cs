...
public class TearManager : MonoBehaviour {
	...
	/// </summary>
	private bool playerMovingPlatformState = false;
	...
	private void Update () 
	{
		...
		{
			Debug.LogError("bad tear being reset");
	...
	private bool PlayerCollidingWithTornPieceCheck()
	{
		for(int itor = 0; itor < MainWorldCutPaper.GetComponent<MeshFilter>().mesh.triangles.Count(); itor++)
		{
			...
			if(GameObject.FindGameObjectWithTag("Player").GetComponent<BoxCollider>().bounds.Contains(vertWorldPos))
			{
				return true;
			}
			/*
	...
	private void UpdateDecalObjects()
	{
		...
		for(int itor = 0; itor < decalObjects.Keys.Count(); itor++)
		{
			...
			if(BadTear)
			{
				decalObjects.Keys.ElementAt(itor).transform.localScale = new Vector3(0.047f, 0.047f, 1.0f);

...
public class TearButtonScript : MonoBehaviour {
	...
	void Update () {
		...
		if(inpManRef.tearMode) gameObject.renderer.material.color = new Color(0.0f, 0.0f, 0.0f);
        ...
        !gameStateManagerRef.GetInputManager().GetFingerGesture().Equals(InputManager.FingerGesture.FOLD)*/
    	{
			...
			inpManRef.foldMode = false;
			inpManRef.tearMode = true;

...
{	
	...
	void Update ()
	{
		//Check for valid input not conflicting with tear -> J.C.
		bool playerInput = false;
		//If we are not currently tear and we are not moving a torn piece, than we can tear
		if(Input.GetMouseButton(1) && !GameObject.FindGameObjectWithTag("TearManager").GetComponent<TearManager>().PlayerMovingPlatformState &&
			!GameObject.FindGameObjectWithTag("TearManager").GetComponent<TearManager>().PlayerCurrentlyTearing)
		{
			playerInput = true;
		}
		else
		...
		if(touchType == TouchType.NONE && !playerInput)
		{
		...
		else if(touchType == TouchType.MULTITOUCH_2 || playerInput)
		{
			if(!firstTouch)
			{
